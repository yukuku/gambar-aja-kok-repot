...
7. **Update the README.md "🚀 Get it now" links.** The section near the top of `README.md` calls out the latest Android APK version number and the web version URL. As part of every release, update the displayed version number so it matches the just-published release. Also verify the web URL (`https://yukuku.github.io/gambar-aja-kok-repot/`) and the "latest release" link still point to the right places whenever the deployment or release setup changes.
